...
@section('title', 'Flyer Ucapan | Rekind')
...
<div class="w-full px-4 md:px-25 py-6 overflow-x-hidden">
    <div class="max-w-full flex justify-between items-start">
        ...
        <div class="w-[662px] overflow-hidden flex flex-col justify-start items-start gap-[10px]">
            <div class="text-[var(--judul)] text-[24px] font-bold leading-[32px] tracking-[0.10px] font-montserrat">Koleksi Flyer Ucapan</div>
            <div class="text-[var(--sub-text)] text-[14px] font-normal leading-[20px] tracking-[0.20px] font-montserrat">Koleksi flyer untuk berbagai ucapan dan hari penting.</div>
        </div>
    ...
    <div class="w-full h-full flex flex-col md:flex-row items-center justify-between gap-y-4 md:gap-[30px]">
        ...
        <div class="w-full max-w-[510px] h-[62px] flex items-center gap-[10px]">
            <form action="{{ route('flyer.search') }}" method="GET" class="w-full">
                <div class="w-full h-[56px] min-w-[280px] max-w-[720px] bg-[var(--highlight-text-box)] overflow-hidden rounded-full flex items-center justify-center gap-[4px]">
                    <div class="flex-1 h-full flex items-center gap-[4px] px-[4px]">
                        ...
                        <div class="flex-1 h-full flex items-center justify-start gap-[10px]">
                            <input type="text" name="search" placeholder="Cari flyer ucapan..." value="{{ $search ?? '' }}" class="w-full bg-transparent border-none outline-none text-[var(--sub-text)] text-[16px] font-roboto font-normal leading-[24px] tracking-[0.5px]">
                        </div>
                        <button type="submit" class="w-[48px] h-[48px] inline-flex flex-col items-center justify-center gap-[10px] cursor-pointer hover:bg-[#C5D7EB] transition-colors rounded-full">
        ...
        <div class="w-full max-w-[510px] overflow-hidden rounded-full flex items-center justify-end gap-[4px]">
            <form id="sort-form" method="GET" class="w-[130px] h-[56px] p-[4px] flex items-center justify-start gap-[4px]">
                <div class="w-[22px] h-[48px] inline-flex flex-col items-center justify-center gap-[10px]"></div>
                <select id="sort-select" name="sort" class="bg-transparent border-none text-[var(--sub-text)] text-[16px] font-roboto font-normal leading-[24px] tracking-[0.5px] cursor-pointer">
        ...
        <div class="mx-auto max-w-screen-xl flex flex-wrap {{ $gridClass }} gap-6">
            ...
            <div class="w-[327px] p-4 bg-white overflow-hidden rounded-[20px] outline-1 outline-[var(--button)] outline-offset-[-1px] flex flex-col gap-[13px]">
                <div class="relative w-full h-[184px] rounded-[16px] overflow-hidden border border-[var(--button)]">
                    <img 
                        src="{{ $flyer->foto ? asset('storage/' . $flyer->foto) : asset('img/pict-4.jpg') }}" 
                        alt="{{ $flyer->nama }}" 
                        class="w-full h-full object-cover" 
                        onerror="this.onerror=null;this.src='{{ asset('img/pict-4.jpg') }}';"
                    />
                </div>
                <div class="flex justify-between items-center">
                    <div class="text-[var(--judul)] text-[18px] font-bold leading-[22px] font-montserrat">{{ $flyer->nama }}</div>
                    <div class="w-[33px] h-[33px] relative">
                        <a href="{{ $flyer->foto ? asset('/files/' . $flyer->foto) : '#' }}" 
                           download 
                           class="cursor-pointer"
                           @if(!$flyer->foto) onclick="event.preventDefault(); alert('File tidak tersedia');" @endif>
                ...
                <div class="h-[16px] flex items-start gap-2">
                    <div class="text-[#BDBDBD] text-[13px] font-normal leading-[16px] font-montserrat truncate">
                        {{ $flyer->deskripsi ? Str::limit($flyer->deskripsi, 100) : 'Flyer ucapan untuk berbagai momen penting' }}
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div class="w-full flex justify-center mt-8">
        {{ $flyers->appends(request()->query())->links('components.pagination') }}
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const searchForm = document.querySelector('form[action="{{ route('flyer.search') }}"]');
        if (searchForm) {
