        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 auto-rows-[330px]">
            @foreach ($photos as $photo)
                <div class="relative group border border-[#7BB7D1] w-full h-full overflow-hidden">
                    <img src="{{ asset('files/' . $photo->foto) }}" class="w-full h-full object-cover transition duration-300" alt="{{ $photo->nama }}" />
...
<div class="w-full px-4 md:px-25 py-6 overflow-x-hidden flex flex-col">
    ...
    <!-- GRID -->
    <div class="w-full px-4">
        <!-- Menggunakan auto-fit dengan minmax untuk menyesuaikan jumlah kolom -->
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
            @foreach ($photos as $photo)
                <div class="relative group border border-[#7BB7D1] overflow-hidden aspect-[4/3] bg-gray-100 flex items-center justify-center">
                    <img 
                        src="{{ asset('files/' . $photo->foto) }}" 
                        class="w-full h-full object-contain transition duration-300" 
                        alt="{{ $photo->nama }}" 
                        loading="lazy"
                    />
                    <div class="absolute inset-0 bg-[var(--button)]/90 flex flex-col items-center justify-center text-center opacity-0 group-hover:opacity-100 transition duration-300 p-4">
                        <h3 class="text-white text-lg font-semibold mb-1">{{ $photo->nama }}</h3>
                        <p class="text-white text-sm">{{ $photo->deskripsi }}</p>
                    </div>
                </div>
    ...
    <div class="w-full flex justify-center mt-8">
        {{ $photos->appends(request()->query())->links('components.pagination') }}
    </div>
</div>
@endsection