...
<div class="w-full overflow-x-hidden">
    <div class="w-full h-full relative bg-white flex flex-col py-4 px-4 gap-3 rounded-md">
        ...
        <div class="w-full flex flex-col md:flex-row justify-between items-center gap-3">
            <form action="{{ route('admin.buletin.search') }}" method="GET" class="w-full md:w-[60%]">
            ...
            <table class="min-w-full text-left text-sm text-gray-700">
                <thead class="bg-gray-100 text-[#1D3A6D] font-bold">
                    <tr>
                        ...
                        <th class="px-4 py-3 border-b">Judul Buletin</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-4 py-3">{{ ($buletins->currentPage() - 1) * $buletins->perPage() + $loop->iteration }}</td>
                        <td class="px-4 py-3">{{ $buletin->nama }}</td>
                        <td class="px-4 py-3">{{ $buletin->tahun }}</td>
                        <td class="px-4 py-3">
                            <div class="flex flex-row">
                                <a href="{{ route('admin.buletin.show', $buletin->id_produk) }}" 
                                ...
                                <a href="{{ route('admin.buletin.edit', $buletin->id_produk) }}" 
                                   class="bg-yellow-300 p-2 border border-l-0 border-r-0">
                                ...
                                <form action="{{ route('admin.buletin.delete', $buletin->id_produk) }}" 
                                      method="POST" 
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-4 py-3 text-center text-gray-500">Tidak ada buletin</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="w-full flex justify-center mt-8">
            {{ $buletins->appends(request()->query())->links('components.pagination') }}
        </div>
    </div>
</div>
