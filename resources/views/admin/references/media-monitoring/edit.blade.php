...
@section('title', 'Edit Media Monitoring')

@section('content')
<div class="w-full overflow-x-hidden">
    <form action="{{ route('admin.media-monitoring.update', $media->id_media) }}" method="POST" class="w-full">
        @csrf
        @method('PUT')
                        ...
                        <div class="relative w-full h-14">
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center h-12">
                                <select name="status" required class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none">
                                    <option value="positive" {{ $media->status == 'positive' ? 'selected' : '' }}>Positive</option>
                                    <option value="negative" {{ $media->status == 'negative' ? 'selected' : '' }}>Negative</option>
                                    <option value="neutral" {{ $media->status == 'neutral' ? 'selected' : '' }}>Neutral</option>
                                </select>
                            </div>
                        ...
                        <div class="relative w-full">
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md">
                                <textarea name="kesimpulan" 
                                          rows="4"
                                          required
                                          class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none resize-none">{{ old('kesimpulan', $media->kesimpulan) }}</textarea>
                            </div>
                            <hr class="border-[1px] text-[var(--judul)]">
                        </div>
                        @error('kesimpulan')
