...
<div class="w-full overflow-x-hidden">
    <div class="w-full h-full relative bg-white flex flex-col py-4 px-4 gap-3 rounded-md">
        <form action="{{ route('admin.buku.update', $book->id_produk) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="flex flex-wrap gap-0">
                    ...
                    <div class="w-full p-6 bg-white rounded-[20px] outline-1 outline-[var(--button)] flex flex-col gap-3">
                        ...
                        <div class="relative w-full h-14">
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center h-12">
                                <input type="text" id="nama" name="nama" value="{{ $book->nama }}" class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none" />
                            </div>
                            ...
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center h-12">
                                <select id="tahun" name="tahun" class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none" required>
                                    @for ($year = 2000; $year <= date('Y'); $year++)
                                        <option value="{{ $year }}" {{ $year == $book->tahun ? 'selected' : '' }}>
                                            {{ $year }}
                                        </option>
                                    @endfor
                                </select>
                    ...
                    <div class="w-full p-6 bg-white rounded-[20px] outline-1 outline-[var(--button)] flex flex-col gap-3">
                        ...
                        <div class="relative w-full">
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center min-h-12">
                                <textarea id="deskripsi" name="deskripsi" class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none py-3" rows="3">{{ $book->deskripsi }}</textarea>
                            </div>
                            <hr class="border-[1px] text-[var(--judul)]">
                        </div>
                        @error('deskripsi')
                ...
                <div class="w-full p-4">
                    <div class="w-full p-6 bg-white rounded-[20px] outline-1 outline-[var(--button)] flex flex-col gap-3">
                        ...
                        <div class="relative w-full">
                            ...
                            @if($book->url)
                                <p class="mt-2 text-sm text-gray-600">File saat ini: {{ basename($book->url) }}</p>
                            @endif
                        </div>
                        @error('url')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="w-full p-4">
                    <div class="w-full p-6 bg-white rounded-[20px] outline-1 outline-[var(--button)] flex flex-col gap-3">
                        <label for="foto" class="text-[var(--judul)] text-lg font-bold leading-[22px]">Ubah Foto Cover Buku</label>
                        <div class="relative w-full">
                            ...
                            @if($book->foto)
                                <div class="mt-2">
                                    ...
                                    <img src="{{ asset('files/' . $book->foto) }}" alt="Cover" class="h-24 object-cover rounded-md">
                                </div>
                            @endif
                        </div>
                        @error('foto')
                ...
                <div class="w-full px-4 flex justify-end gap-3">
                    ...
                    <a href="{{ route('admin.buku.index') }}" class="cursor-pointer bg-[var(--line)] text-white font-semibold py-2 px-6 rounded-xl hover:bg-[#cc6512] transition inline-block">
                        Cancel
                    </a>                    
                </div>
            </div>
