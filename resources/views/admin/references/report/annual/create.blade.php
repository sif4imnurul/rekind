...
@section('title', 'Tambah Annual Report')

@section('content')
<div class="w-full overflow-x-hidden">
    <div class="w-full h-full relative bg-white flex flex-col py-4 px-4 gap-3 rounded-md">

        <form action="{{ route('admin.annual.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="flex flex-wrap gap-0">
                            ...
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center h-12">
                                <select id="tahun" name="tahun" class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none">
                                    @for($year = date('Y'); $year >= 1900; $year--)
                                        <option value="{{ $year }}" {{ old('tahun', $annualReport->tahun ?? '') == $year ? 'selected' : '' }}>
                                            {{ $year }}
                                        </option>
                                    @endfor
                                </select>
                ...
                <div class="w-full px-4 flex justify-end gap-3">
                    ...
                    <a href="{{ route('admin.annual.index') }}" class="bg-[var(--line)] text-white font-semibold py-2 px-6 rounded-xl hover:bg-[#cc6512] transition text-center">
                        Cancel
                    </a>
                </div>
