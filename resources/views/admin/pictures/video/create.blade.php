        ...
        <form action="{{ route('admin.video.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex flex-wrap gap-0">
                    ...
                    <div class="w-full p-6 bg-white rounded-[20px] outline-1 outline-[var(--button)] flex flex-col gap-3">
                        ...
                        <div class="relative w-full">
                            <textarea name="deskripsi" 
                                      required
                                      rows="3"
                                      class="w-full bg-[var(--highlight-text-box)] p-4 text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none rounded-t-md">{{ old('deskripsi') }}</textarea>
                            <hr class="border-[1px] text-[var(--judul)]">
                        </div>
                        @error('deskripsi')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="w-full p-4">
                    <div class="w-full p-6 bg-white rounded-[20px] outline-1 outline-[var(--button)] flex flex-col gap-3">
                        <label class="text-[var(--judul)] text-lg font-bold leading-[22px]">Unggah Video</label>
                        <div class="relative w-full">
                            <input type="file" 
                                   name="video"
                                   accept="video/*"
