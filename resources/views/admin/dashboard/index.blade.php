...
<div class="p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-4 rounded-xl shadow flex items-center gap-4">
            ...
            <div>
                <p class="text-sm text-gray-500">Foto</p>
                <p class="text-lg font-bold text-[var(--judul)]">{{ $countFoto }}</p>
            </div>
        </div>
        <div class="bg-white p-4 rounded-xl shadow flex items-center gap-4">
            <div class="bg-green-100 p-3 rounded-full">
            ...
            <div>
                <p class="text-sm text-gray-500">Video</p>
                <p class="text-lg font-bold text-[var(--judul)]">{{ $countVideo }}</p>
            </div>
        </div>
        <div class="bg-white p-4 rounded-xl shadow flex items-center gap-4">
            <div class="bg-yellow-100 p-3 rounded-full">
            ...
            <div>
                <p class="text-sm text-gray-500">Twibbon</p>
                <p class="text-lg font-bold text-[var(--judul)]">{{ $countTwibbon }}</p>
            </div>
        </div>
        <div class="bg-white p-4 rounded-xl shadow flex items-center gap-4">
            <div class="bg-red-100 p-3 rounded-full">
            ...
            <div>
                <p class="text-sm text-gray-500">Data Client</p>
                <p class="text-lg font-bold text-[var(--judul)]">{{ $countClient }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <a href="{{ route('admin.buku.create') }}" class="bg-[var(--button)] hover:bg-[var(--sub-judul)] text-white p-4 rounded-xl text-center font-semibold transition flex items-center justify-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 22h15v-2H6.012C5.55 19.988 5 19.805 5 19s.55-.988 1.012-1H21V4c0-1.103-.897-2-2-2H6c-1.206 0-3 .799-3 3v14c0 2.201 1.794 3 3 3zM5 8V5c0-.805.55-.988 1-1h13v12H5V8z"></path><path d="M8 6h9v2H8z"></path></svg>
                Tambah Buku
            </a>
        </div>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <ul class="space-y-3">
            ...
            <li class="flex items-center justify-between {{ !$loop->last ? 'border-b pb-2' : '' }}">
                <div>
                    <p class="font-semibold text-gray-700">{{ $update->nama }}</p>
                    <p class="text-xs text-gray-400">{{ $update->created_at->diffForHumans() }}</p>
                </div>
                <a href="#" class="text-blue-500 text-sm">Lihat ➔</a>
            </li>
            @empty
            <li class="text-center text-gray-500">Belum ada update terbaru</li>
            @endforelse
        </ul>
    </div>
</div>
