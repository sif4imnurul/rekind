...
<div class="w-full overflow-x-hidden">
    <div class="w-full h-full relative bg-white flex flex-col py-4 px-4 gap-3 rounded-md">
        <form action="{{ route('admin.crm.data-client.update', $client->id_client_data) }}" method="POST">
            @csrf
            @method('PUT')
                                ...
                                <select name="id_user" 
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id_user }}" 
                                                {{ old('id_user', $client->id_user) == $user->id_user ? 'selected' : '' }}>
                        ...
                        <div class="relative w-full">
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center">
                                <textarea 
                                    ...
                                    rows="3"
                                    class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none">{{ old('alamat_perusahaan', $client->alamat_perusahaan) }}</textarea>
                            </div>
                            <hr class="border-[1px] text-[var(--judul)]">
                        </div>
                        ...
                        <div class="relative w-full h-14">
                            <div class="bg-[var(--highlight-text-box)] px-4 py-1 rounded-t-md flex items-center h-12">
                                <select name="status_project" 
                                        required
                                        class="w-full bg-transparent text-[var(--judul)] text-base tracking-wide font-normal focus:outline-none">
                                    <option value="current" {{ old('status_project', $client->status_project) == 'current' ? 'selected' : '' }}>Current</option>
                                    <option value="past" {{ old('status_project', $client->status_project) == 'past' ? 'selected' : '' }}>Past</option>
                                    <option value="potential" {{ old('status_project', $client->status_project) == 'potential' ? 'selected' : '' }}>Potential</option>
                                </select>
                            </div>
                            <hr class="border-[1px] text-[var(--judul)]">
                        </div>
